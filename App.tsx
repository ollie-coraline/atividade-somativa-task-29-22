...
import { getSessionToken, saveSessionToken, removeSessionToken } from './src/utils/storage';

// TODO (Zustand): Importe o seu useTaskStore aqui

type AppScreen = 'login' | 'signup' | 'tasks';
...
export default function App() {
  ...
  // Salvar o token quando ele for alterado
  useEffect(() => {
    if (sessionToken && !isInitializing) {
      saveSessionToken(sessionToken).catch(error => {
        console.error('Erro ao salvar token:', error);
      });
    }
  }, [sessionToken, isInitializing]);
  ...
  const handleLogout = async () => {
    try {
      // TODO: Chamar a função logout da API se necessário
  ...
  // Mostrar tela de carregamento enquanto verifica se há um token salvo
  if (isInitializing) {
    return (
  ...
  // Se o usuário não está autenticado, mostrar telas de login/signup
  if (!sessionToken) {
    if (currentScreen === 'signup') {
      return <SignupScreen onNavigateToLogin={handleNavigateToLogin} onSignupSuccess={handleSignupSuccess} />;
    }
    return <LoginScreen onNavigateToSignup={handleNavigateToSignup} onLoginSuccess={handleLoginSuccess} />;
  }
